...
def worker_execute(args):
    ...
    print("python script_eval_on_zoo_topologies.py -t "+str(tm_id)+" -m "+str(model_id)+" -g "+graph_topology_name+" -o "+drl_eval_res_folder+" -d "+differentiation_str+ ' -f ' + dataset_folder)
    subprocess.call(["python script_eval_on_zoo_topologies.py -t "+str(tm_id)+" -m "+str(model_id)+" -g "+graph_topology_name+" -o "+drl_eval_res_folder+" -d "+differentiation_str+ ' -f ' + dataset_folder], shell=True)
...
if __name__ == "__main__":
    ...
    dataset_folder = "../results-1-link_capacity-unif-05-1-zoo/"
    # In this folder we store the rewards that later will be parsed for plotting
    drl_eval_res_folder = "../rwds-results-1-link_capacity-unif-05-1-zoo/"
